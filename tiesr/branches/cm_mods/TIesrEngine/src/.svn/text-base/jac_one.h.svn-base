...
/*
 Estimate of SVA weighting factor multiplier prior based on quadratic polynomial
 function of SNR level.  These are based on experiment and may need to be
 changed according to the MFCC configuration. MFCCs up to START_DIMENSION do not
 have SVA update, so have no associated polynomial. Currently polynomials 
 for variance multiplier scaling priors are specified for MFCC[1-9] and
 delta_MFCC[0-9].

 Polynomial is:
 poly[0] * x * x + poly[1] * x + poly[2]
   
   poly[0] in Q15,
   poly[1] in Q12, 
   poly[2] in Q12 
*/
static const short MAX_DIM_WSVA_STATIC = 9;
static const short MAX_DIM_WSVA_DELTA = 9;
static const short wsva_polycoef[19][3] = { { -62 , 720, -556 } , {4, 89, 276 }, { 43, -208, 5945} ,

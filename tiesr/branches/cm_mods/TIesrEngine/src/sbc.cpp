...
static short sbc_prior_invvar(short dim, short n_mfcc )
{

  /* If static prior estimate exists, return it */
  if( dim < n_mfcc && dim <= MAX_DIM_SBC_STATIC )
     return SBC_PRIOR_INVVAR[dim];

  /* If delta prior estimate exists, return it */
  else if( dim >= n_mfcc && dim <= n_mfcc + MAX_DIM_SBC_DELTA )
     return SBC_PRIOR_INVVAR[dim - n_mfcc + MAX_DIM_SBC_STATIC + 1];

  /* Otherwise return last delta prior as a default */
  else
     return SBC_PRIOR_INVVAR[ MAX_DIM_SBC_STATIC + 1 + MAX_DIM_SBC_DELTA ];
}
...
static ushort sbc_update_SBC_seg(ushort hmm_code, ushort cep_dim, ushort n_mfcc,
...
#ifdef SBC_PRIOR
      ltmp2 = sbc_prior_invvar(jx, n_mfcc);
...
TIESRENGINECOREAPI_API void sbc_update_SBC(gmhmm_type* gv)
{
  ...
  for (s = gv->nbr_seg -1; s >=0 ; s--){ /* for each segment */
    sbc_update_SBC_seg(gv->hmm_code[s], gv->nbr_dim, gv->n_mfcc,

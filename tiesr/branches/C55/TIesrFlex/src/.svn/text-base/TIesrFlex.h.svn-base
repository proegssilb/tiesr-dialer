 ...
 * TIesrFlex.h
 *
 * Header defining the interface to the TIesrFlex and the TIesrFlex class.
 *
...
class TIESRFLEX_API CTIesrFlex
{
   ...
   public:
      ...
      /*----------------------------------------------------------------
       ...
       format. Set useFlags to include the AddClosure flag if closure phones
       (bcl, dcl, gcl, kcl, pcl, tcl) are not to be automatically added in
       the dictionary. Set useFlags to include NoDTLookup to bypass using
       decision trees and only use the dictionary.
       ----------------------------------------------------------------*/
      Errors LoadLanguage( const char *direct, const char *language,
      const char *dict_ident, const char *hmm_ident,
      const int lit_end, const int useFlags );
   ...
   private:
      ...
      typedef unsigned char Pron_t[MAX_PRON];

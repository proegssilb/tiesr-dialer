					  ...
					  ushort useglength, char bUpdate)
{
    ...
    sbc_get_reliable_node(iTerminalIdx, sa);
    return iTerminalIdx; 
}
					    ...
					    Boolean bTriphone )
{
   ...
   if (bTriphone){
     ...
     nread = my_fread( sa->pHMM2phone, sizeof(short), n_hmms, fp );
     my_fclose( fp );
...
TIESRENGINECOREAPI_API TIesrEngineHLRStatusType sbc_save(FeaHLRAccType* sa, FILE *pf)
{
  ...
  if (sa && pf){
    my_fwrite(&(tree->n_utt), sizeof(ushort), 1, pf);
    nwrite = my_fwrite(&(tree->n_nodes), sizeof(ushort), 1, pf);
    if( nwrite != 1 )
      return eTIesrEngineHLRFail;
    nwrite = my_fwrite(&(sa->hlr_TRANSFORM_SIZE), sizeof(ushort), 1, pf);
    if( nwrite != 1 )
      return eTIesrEngineHLRFail;
    
    for (ix=0;ix<tree->n_nodes;ix++){
      my_fwrite(&(tree->node[ix]->vector_count), sizeof(ushort), 1, pf);
      my_fwrite(&(tree->node[ix]->parent_node_and_transform_idx), sizeof(short), 1, pf);
      p_trans = sbc_get_transform(ix, sa); 
      dif1 = sbc_get_transform_stat_dif1(ix, sa); 
      dif2 = sbc_get_transform_stat_dif2(ix, sa); 
      my_fwrite(dif1, sizeof(long), cep_dim, pf); 
      my_fwrite(dif2, sizeof(long), cep_dim, pf); 
      my_fwrite(p_trans, sizeof(short), cep_dim,pf);
    }
    
    /* save Terminal transformation index information */
    my_fwrite(&(sa->n_monophones), sizeof(ushort), 1, pf);
    nwrite = my_fwrite(sa->hlrTerminal, sizeof(ushort), sa->n_monophones, pf); 
    if( nwrite != sa->n_monophones) return eTIesrEngineHLRFail;
    
    nwrite = my_fwrite(sa->hlrTransformIndex, sizeof(ushort), tree->n_nodes, pf); 
    if (nwrite != tree->n_nodes) return  eTIesrEngineHLRFail;
    
    pShortObj = sa->hlrTransforms; 
    nwrite = my_fwrite(pShortObj, sizeof(short), 
...
TIesrEngineHLRStatusType sbc_save(FeaHLRAccType* sa , FILE* pf)
{
  ...
  if (sa && pf){
     nwrite = my_fwrite(&(sa->mem_count), sizeof(ushort), 1, pf);
     if( nwrite != 1 )
	return eTIesrEngineHLRFail;

     nwrite = my_fwrite(sa->hlrTreeBase, sizeof(short), sa->mem_count, pf);
...
TIESRENGINECOREAPI_API TIesrEngineHLRStatusType sbc_load(FeaHLRAccType* sa, FILE *pf)
{
   ...
   if (sa && pf){
      if (!feof(pf)){
	nread = my_fread(&utmp, sizeof(ushort), 1, pf);
	...
	for (ix=0;ix<tree->n_nodes;ix++){
	  if (bOverWrite) {
	    nread = my_fread(&(tree->node[ix]->vector_count), sizeof(ushort), 1, pf);
	    if (nread != 1) return eTIesrEngineHLRLoadTree;
	  }
	  else 
	    my_fseek(pf, sizeof(ushort), SEEK_CUR); 

	  if (bOverWrite) {
	    nread = my_fread(&(tree->node[ix]->parent_node_and_transform_idx), 
			  sizeof(short), 1, pf);
	    if (nread != 1) return eTIesrEngineHLRLoadTree;
	  }else 
	    my_fseek(pf, sizeof(short), SEEK_CUR); 

	  if (bOverWrite){
	    dif1 = sbc_get_transform_stat_dif1(ix, sa); 
	    dif2 = sbc_get_transform_stat_dif2(ix, sa); 
	    p_trans = sbc_get_transform(ix, sa); 
	    nread = my_fread(dif1, sizeof(long), sCepDim, pf); 
	    if (nread != (size_t)sCepDim) return eTIesrEngineHLRLoadTree;
	    nread = my_fread(dif2, sizeof(long), sCepDim, pf); 
	    if (nread != (size_t)sCepDim) return eTIesrEngineHLRLoadTree;
	    nread = my_fread(p_trans, sizeof(short), sCepDim, pf);
	    if (nread != (size_t)sCepDim) return eTIesrEngineHLRLoadTree;
	  }else
	    my_fseek(pf, sizeof(long)*(sCepDim << 1) + sizeof(short)*sCepDim, SEEK_CUR); 
	}
	  
	if (bOverWrite){
	  /* read Terminal transformation index information */
	  my_fread(&(sa->n_monophones), sizeof(ushort), 1, pf); 
	  nread = my_fread(sa->hlrTerminal, sizeof(ushort), sa->n_monophones, pf); 
	  if( nread != sa->n_monophones) return eTIesrEngineHLRLoadTree;
	  
	  nread = my_fread(sa->hlrTransformIndex, sizeof(ushort), tree->n_nodes, pf); 
	  if (nread != tree->n_nodes) return  eTIesrEngineHLRLoadTree;
	  
	  pShortObj = sa->hlrTransforms; 
	  nread = my_fread(pShortObj, sizeof(short), 
			tree->n_nodes * sa->hlr_TRANSFORM_SIZE, pf); 
	  if (nread != (size_t) tree->n_nodes * sa->hlr_TRANSFORM_SIZE) 
	    return eTIesrEngineHLRLoadTree;
	}else
	  my_fseek(pf, sizeof(short)*(1 + sa->n_monophones + tree->n_nodes + tree->n_nodes*sa->hlr_TRANSFORM_SIZE), SEEK_CUR); 
					...
					Boolean bTriphone )
{
   ...
   nread = my_fread( &(sa->n_terminals), sizeof(ushort), 1, fp );
   if( nread != 1 )
      goto InitFail;
   ...
   if (!load_monophone_only) 
   {
	  ...
	  /*   			    "tree" ); */
       sa->hlrTreeBase = (short*) malloc(  sizeof(sbcTree) ); // Unsure with that, M. Schuster
	 ...
	 goto InitMemoryFail;
	  ...
	  /*   			      SBC_SIZE, LONGALIGN,  */
	  /*   			      "node"); */
      tree->node = (psbcTreeNode*) malloc(  (tree->n_nodes*sizeof(psbcTreeNode)));
      if( tree->node == NULL )
	 goto InitMemoryFail;
   }
   ...
   for( i = 0; i <  tree->n_nodes; i++)
   {
	  ...
	  /*   				 SBC_SIZE, LONGALIGN,  */
	  /*   				 "node"); */
       tree->node[i] = (psbcTreeNode)  malloc( sizeof(sbcTreeNode));
       if( tree->node[i] == NULL )
	 goto InitMemoryFail;

      nread = my_fread( &p_node, sizeof(ushort), 1, fp );
      if( nread != 1 )
	 goto InitFail;
	  ...
	  /*   				SBC_SIZE, LONGALIGN,  */
	  /*   				"diff 1"); */
      treenode->p_acc_dif1 = (long*) malloc( sa->hlr_TRANSFORM_SIZE *sizeof(long));
	 ...
	 goto InitMemoryFail;
	  ...
	  /*   				SBC_SIZE, LONGALIGN,  */
	  /*   				"diff 2"); */
      treenode->p_acc_dif2 = (long*) malloc( sa->hlr_TRANSFORM_SIZE *sizeof(long));
   ...
   nread = my_fread( &n_monophones, sizeof( ushort), 1, fp );
   if( nread != 1 )
      goto InitFail;
   ...
   /*  				   SBC_SIZE, SHORTALIGN,  */
   /*  				   "monophone list" ); */
   sa->hlrTerminal = (ushort *) malloc(  n_monophones*sizeof(ushort));
   ...
   nread = my_fread( utmp, sizeof(ushort), n_monophones, fp );
   ...
   sa->pSegStat_dif1 = (short*)  malloc(  cep_dim*sizeof(cep_dim));
   ...
   sa->pSegStat_dif2 = (short*) malloc(  cep_dim*sizeof(short));
   ...
   /*  					 SBC_SIZE, SHORTALIGN,  */
   /*  					 "transform index" ); */
   sa->hlrTransformIndex = (ushort *) malloc( tree->n_nodes*sizeof(ushort));
   ...
   /*  				   SBC_SIZE, SHORTALIGN,  */
   /*  				   "bias" ); */
   sa->hlrTransforms = (short*) malloc( tree->n_nodes * sa->hlr_TRANSFORM_SIZE*sizeof(short));
   ...
   /*  			       SBC_SIZE, SHORTALIGN,  */
   /*  			       "enhanced feature" ); */
   sa->pEnhanced = (short*) malloc(tree->n_nodes * sa->hlr_TRANSFORM_SIZE*sizeof(short));
   ...
   /*  				"backup node cnt"); */
   sa->bk_node_cnt = (short*) malloc(tree->n_nodes*sizeof(short));
...
#else
      mfcc_feature = (short*)gv->mem_feature + 2*crt_vec;
#endif

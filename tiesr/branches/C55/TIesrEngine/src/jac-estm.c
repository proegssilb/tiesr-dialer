			  ...
			  short * p_transform, short s_normalization)
{
  ...
  FOR_EMS_STATES(j,jac_hmm, gv->base_tran) {
	...
	log_spectral_compensation(sp_log_ps, (short *) NULL, noisy_log, (short *) NULL, log_N, log_H, 1, func_d, gv->n_filter);
...
#if defined( USE_SVA ) || defined( USE_SBC )
				 ...
				 short n_mfcc, gmhmm_type *gv)
{
  ...
  short *alfa_t, *beta_t;
   ...
   ushort feat_size = n_mfcc;
#else
   short *mfcc_feature;
   ushort feat_size = gv->nbr_dim;
#endif
  ...
  for (seg = token * feat_size, t=0; t<T; t++, seg += feat_size) {
...
#else
    mfcc_feature = (short*)gv->mem_feature + seg;
#endif
				      ...
				      Boolean bTriphone)
{
   ...
   if( pf )
   {
      ...
      numRead = my_fread( &numFilters, sizeof(short), 1, pf );
      if( numRead != 1 || numFilters != gv->n_filter )
	{
      ...
      numRead = my_fread( &featDim, sizeof(short), 1, pf );
	...
	goto Failure;
      ...
      numRead = my_fread( gv->log_H, sizeof(short), gv->n_filter, pf );
      if( numRead != gv->n_filter )
	{
      ...
      numRead = my_fread( gv->accum, sizeof(long), 2*gv->n_filter, pf );
      if( numRead != (size_t)2*gv->n_filter )
	{
      ...
      numRead = my_fread( gv->log_var_rho, sizeof(short), gv->nbr_dim, pf );
      if( numRead != (size_t)gv->nbr_dim )
	{
      ...
      numRead = my_fread( gv->var_fisher_IM, sizeof(long), gv->nbr_dim, pf );
      if( numRead != (size_t)gv->nbr_dim )
	{
	  eStatus = eTIesrEngineJACLoadFail;
	  goto Failure;
      ...
      for (d = 0; d < gv->nbr_dim; d++) 
	{
      ...
      if (pNss){
	pNss->prevMeanEn = 0;
	numRead = my_fread(&(pNss->prevMeanEn), sizeof(short), 1, pf);
	if( numRead != 1 )
	  {
	    eStatus = eTIesrEngineJACLoadFail;
	    goto Failure;
  ...
  Failure:
   ...
   if (eStatus == eTIesrEngineJACLoadFail) pf = (FILE*)NULL; 

   /* Reinitialize JAC if no JAC state file or load fail */
   if (eStatus != eTIesrEngineJACSuccess || pf == (FILE*) NULL) 
...
{ 
   ...
   {
      numWritten = my_fwrite( &gv->n_filter, sizeof(ushort), 1, pf );
      if( numWritten != 1 )
	 goto Failure;

      numWritten = my_fwrite( &gv->nbr_dim, sizeof(short), 1, pf );
      if( numWritten != 1 )
	 goto Failure;

      numWritten = my_fwrite(gv->log_H, sizeof(short), gv->n_filter, pf);
      if( numWritten != gv->n_filter )
	goto Failure;
      
      numWritten = my_fwrite( gv->accum, sizeof(long), 2*gv->n_filter, pf );
      if( numWritten != (size_t)2*gv->n_filter )
	goto Failure;

#ifdef USE_SVA
      /* Write the SVA log_var_rho parameter */
      numWritten = my_fwrite( gv->log_var_rho, sizeof(short), gv->nbr_dim, pf );
      if( numWritten != (size_t)gv->nbr_dim )
	goto Failure;
      
      /* Read the SVA Fisher IM parameter */
      numWritten = my_fwrite( gv->var_fisher_IM, sizeof(long), gv->nbr_dim, pf );
      if( numWritten != (size_t)gv->nbr_dim )
	goto Failure;
#endif      
	 ...
	 level of the previous utterance in the next iteration */
      if (pNss) {
	numWritten = my_fwrite(&pNss->noiselvl, sizeof(short), 1, pf);
	if( numWritten != 1)
	  goto Failure;
      }
#endif      
...
#else
   short *mfcc_feature = (short *)gv->mem_feature + start_frame * gv->nbr_dim;
#endif
...
static TIesrEngineStatusType jac_force_alignment(gmhmm_type * gv)
{
  ...
  if ( status == eTIesrEngineSuccess ) search_a_frame((short*)NULL, 0, total_frm - 1, gv ); 

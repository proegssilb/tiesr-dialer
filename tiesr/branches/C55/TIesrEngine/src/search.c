...
{
  short i_pos = -1; 
  StateCell *pstate = (StateCell *)NULL; 
...
#define SET_NEXT_SYM(sym_cell,ps,direction, nbr) {sym_cell = ps; if ( --(nbr) ) ps += direction; else ps = (SymCell *) NULL; }
...
path_propagation(short cur_idx, short prev_idx, unsigned short frm_cnt, gmhmm_type *gv, short prune)
{
   ...
   SymCell        *sym_prev, *ps0 = (SymCell *)NULL, *ps1 = (SymCell *)NULL;
   HmmType        *hmm;
  ...
  update_obs_prob(short mfcc_feature[], short cur_idx, unsigned short frm_cnt, gmhmm_type *gv, unsigned short last_frm)
{
  ...
  SymCell        *sym_cell;
  SymType *psym;
  SymCell *ps0 = (SymCell *)NULL, *ps1 = (SymCell *)NULL;  
  HmmType        *hmm;

  short          gs,  sym_active, direction, status, n_state, *trnprob, max_exit_scr, first_time_call = TRUE, *sObs = (short*) NULL;
  ...
  long           scr;
  StateCell      *max_exit_state = (StateCell*) NULL, *state_cell, *start_cell;
...
init_search(short mfcc_feature[], short cur_idx, gmhmm_type *gv, unsigned short last_frm)
{
  ...
  short          gs, max_exit_scr, *p_enh = (short*)NULL;
  long           scr;
   ...
   if (gv->word_backtrace==WORDBT)
   {
      ...
      if ( frm >= 0 )
      {
         ...
         do
         {
            ...
            unsigned short gotit;
            unsigned short wordend;
            unsigned short t_sym2;
   ...
   {//hmm backtrace
      ...
      if ( frm >= 0 )
      {
         ...
         do
         {
            ...
            {
               // EDIT: DOES THIS HARM I changed here, Michael Schuster
               symcode = SHRT_MAX;
               frm = SHRT_MAX;
            }

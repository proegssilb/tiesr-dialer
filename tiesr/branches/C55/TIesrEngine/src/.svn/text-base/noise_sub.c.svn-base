...
#ifdef PRT_TRACE
  if (count_frm == 0) {
    psm = my_fopen("SMOf.d","w");
    pns = my_fopen("NYSf.d","w");
    psi = my_fopen("SIGIf.d","w");
    pso = my_fopen("SIGOf.d","w");
  }
#endif
...
{
  /* ns->mem_count = 0;  */
  /* ns->s_noise_floor =(short*) mem_alloc(ns->base_mem, &ns->mem_count,  */
  /*   			NSS_HALF_WINDOW, NSS_HALF_WINDOW * 3,  */
  /*   			SHORTALIGN,  */
  /*   			"noise floor" ); */
  ns->s_noise_floor =(short*) malloc(NSS_HALF_WINDOW *sizeof(short));

  /* ns->snr = (short*) mem_alloc(ns->base_mem, &ns->mem_count,  */
  /*   	      NSS_HALF_WINDOW, NSS_HALF_WINDOW * 3,  */
  /*   	      SHORTALIGN,  */
  /*   	      "noise floor" ); */

  ns->snr = (short*) malloc(NSS_HALF_WINDOW *sizeof(short));


  /* ns->ps_long_term_noise =(short*) mem_alloc(ns->base_mem, &ns->mem_count,  */
  /*   			     NSS_HALF_WINDOW, NSS_HALF_WINDOW * 3,  */
  /*   			     SHORTALIGN,  */
  /*   			     "noise floor" ); */
  ns->ps_long_term_noise =(short*) malloc(NSS_HALF_WINDOW *sizeof(short));
  ss_set_default(ns);
  ss_init( ns ); 
}
...
TIESRENGINECOREAPI_API void ss_close(gmhmm_type * gv)
{
  ...
  my_fclose(p_ss_crtl->fp);
  my_fclose(p_ss_crtl->fp2);
  my_fclose(p_ss_crtl->fp3);
#endif
}

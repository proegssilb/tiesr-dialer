 ...
 * TIesrFlex.cpp
 *
 * TIesrFlex API - Create models and grammar network for TIesr.
 *
        ...
        const int useFlags )
{
   ...
   dError = m_dict->LoadDictionary( direct, language, dict_ident, useFlags );
        ...
        int aAutoSilence )
{
   ...
   // If user requests no pronunciations, ie, rules only, but no rules
   // available, return an error
   if( aMaxPronunciations == 0 && ( m_useFlags & NoDTLookup ) )
   {
      return ErrNoRules;
   }
   else
   ...
   // User requested rule pronunciations only honored if decision tree
   // lookup is used.
   if( aIncludeRulePron && ( m_useFlags & NoDTLookup ) )
   {
      return ErrNoRules;
   }
   else
...
int CTIesrFlex::yylex( long *yylval )
{
   ...
   while( true )
   {
      ...
      else if( myisalpha( (unsigned int) * cfg_char ) || *cfg_char == '-'
              || *cfg_char == '\'' || *cfg_char == '_' )
      {
         while( myisalpha( (unsigned int) * cfg_char ) || *cfg_char == '-'
                 || *cfg_char == '\'' || *cfg_char == '_' )
         {
...
void CTIesrFlex::create_sym_net( int aSentNode )
{
   ...
   while( tran )
   {
      ...
      for( i = pron[0] - 1; i >= 0; i-- )
      {
         ...
         if( m_bInputMonophone || c == SIL_INDEX )
         {
            l = SIL_INDEX;
            r = SIL_INDEX;
         }
...
void CTIesrFlex::create_hmm( )
{
   ...
   /* hmm[0] is sil */
   add_hmm_entry( SIL_INDEX, SIL_INDEX, SIL_INDEX );
   ...
   for( baseIndex = 0; baseIndex < m_baseWordIndex; baseIndex++ )
   {
      ...
      if( strncmp( word[baseIndex], SIL, silLen ) == 0 )
      {
         ...
         m_wordProns[baseIndex][1] = SIL_INDEX;
...
CTIesrFlex::Errors CTIesrFlex::LookupBasePronunciation( const char* aWord, char* aPron )
{
   ...
   else
   {
      ...
      // but no dictionary pronunciation exists.  This may not be
      // an error, since we just have to use the rule pronunciation,
      // if it is available.
      if( dictError == CTIesrDict::ErrNotInDictionary && aPron[0] != 0 )
